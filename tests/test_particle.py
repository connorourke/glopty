...
class ParticleTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_particle = Mock(spec=Particle, vector=self.vector, fit=self.fit, index=self.index)
    ...
    def test_vector_convert(self):
        ...
        self.assertEqual(self.particle.vector_to_pos(bounds).all(),self.particle.pos_to_vector(self.particle.vector_to_pos(bounds),bounds).all())
